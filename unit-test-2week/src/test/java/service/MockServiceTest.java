...
//        System.out.println("챔피언 이름: "+champion.getName());
    ...
    public void List객체사이즈검증테스트(){
        ...
        champions.add(new Champion("티모","미드",3));
        champions.add(new Champion("유미","정글",3));
    ...
    public void 가짜챔피언객체리턴테스트(){
        when(mockService.findByName(anyString())).thenReturn(new Champion("애쉬","미드",3));
        assertThat(mockService.findByName("유미").getHasSkinCount(),is(3));
        verify(mockRepository,times(1)).findByName(anyString());
    }

    // 4. 2개 이상의 가짜 챔피언 객체를 List로 만들어 리턴하고, 하나씩 해당 객체를 검색한 뒤 검색을 위해 호출한 횟수를 검증하세요.
    @Test
    public void 여러개개이상호출회수검증(){
        List<Champion> champions = new ArrayList<>();
        champions.add(new Champion("티모","미드",3));
